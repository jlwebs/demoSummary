                 ...
                 const char *method, const char *query_string)
{
	printf("cgi excute\n");
	 ...
	 if (pid == 0)  /* 子进程: 运行CGI 脚本 */
	 {
		  ...
		  dup2(cgi_output[1], 1);
		  dup2(cgi_input[0], 0);
		  //标准输入文件的文件描述符： 0    标准输出文件的文件描述符：1    标准错误输出的文件描述符：2
		//.cgi : postdata = sys.stdin.read(int(length))

		  close(cgi_output[0]);//关闭了cgi_output（子进程）中的读通道
		  close(cgi_input[1]);//关闭了cgi_input（父进程）中的写通道
		...
		//getpid()来获取自己进程的id
		  ...
		  close(cgi_output[1]);//关闭了cgi_output（子进程）中的写通道
		  close(cgi_input[0]);//关闭了cgi_input（父进程）中的读通道 =>父子进程管道通信全关闭了
			 ...
			 for (i = 0; i < content_length; i++) 
			   {
				...
				write(cgi_input[1], &c, 1);//走父进程的写通道写入1字节
			   }
